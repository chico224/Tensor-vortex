...
flowchart TD
    ...
    subgraph CoreEngine ["🧠 Couche d'Intelligence Quantique"]
        ...
        QEA(["Recuit Quantique<br/>(Entropie Thermique CPU)"]) -.->|Pondérations Statistiques| CONSENSUS
    end
...
```mermaid
flowchart TD
    Signal["🔥 Signal Achat/Vente Validé (Consensus)"] --> EquityCheck(["💰 Scan Type de Compte<br/>(Standard, Cent, ECN)"])
    
    EquityCheck -->|Taille d'Équité Lue| RiskCalc(["🛡️ Règle des 1%<br/>Calcul Dynamique du Lot (Lot Sizing)"])
    RiskCalc --> LimitCheck{"🛑 Filtre Anti-Crash (Risk Manager)"}
    ...
    LimitCheck -->|Sécurité OK| TP_SL["📐 Calcul Multi-ATR<br/>Fixation du Stop Loss (SL) & Take Profit (TP)"]
    ...
    Finalize -->|Trade Gagnant| Win(["✅ Addition au Solde<br/>Mise à jour de la Facture SaaS (3%)"])
    Finalize -->|Trade Perdant| Loss(["⛔ Réduction du Solde<br/>Incrémentation Compteur de Draws"])
